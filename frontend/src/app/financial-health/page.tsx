...
export default function FinancialHealthPage() {
  ...
  const bentoItems = [
    // Header
    {
      ...
      content: (
        <div className="p-6">
          <h1 className="text-3xl font-bold text-foreground">Financial Health</h1>
          <p className="text-muted-foreground mt-1">Monitor your startup's financial well-being</p>
        </div>
      ),
    },
        ...
        <div className="p-6 h-full flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-lg font-medium text-foreground">Health Score</h3>
            </div>
    ...
    // Runway
    {
      ...
      background: <SpotlightCard spotlightColor={runway < 6 ? 'rgba(239, 68, 68, 0.3)' : 'rgba(34, 197, 94, 0.3)'} className="w-full h-full" />,
      content: (
        <div className="p-6">
          <div className="flex items-center gap-3 mb-2">
            <div className={`h-10 w-10 rounded-lg ${runway < 6 ? 'bg-red-500/10 text-red-500' : 'bg-emerald-500/10 text-emerald-500'} flex items-center justify-center`}>
    ...
    // Burn Rate
    {
      ...
      content: (
        <div className="p-6">
          ...
          <p className="text-xs text-muted-foreground mt-2">Average monthly expenses</p>
        </div>
      ),
    },
    ...
    // Runway Trend
    {
      ...
      content: (
        <div className="p-6 h-full flex flex-col">
          <div className="mb-4">
            <h3 className="text-lg font-medium text-foreground">Runway Trend</h3>
            <p className="text-sm text-muted-foreground">Months of runway over time</p>
          </div>
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {insights.positives.map((item: string, i: number) => (
                    <li key={i} className="text-sm text-muted-foreground pl-6 relative before:content-['•'] before:absolute before:left-2">
                      {item}
                    </li>
                  ))}
                </ul>
    ...
    // Burn Rate Trend
    {
      ...
      content: (
        <div className="p-6 h-full flex flex-col">
          <div className="mb-4">
            <h3 className="text-lg font-medium text-foreground">Burn Rate & Cash Balance</h3>
          ...
          <div className="flex-grow min-h-[250px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                ...
                />
                <Line type="monotone" dataKey="burnRate" stroke="#ef4444" strokeWidth={2} name="Burn Rate" />
    ...
    // Financial Ratios
    {
      ...
      content: (
        <div className="p-6">
          <h3 className="text-lg font-medium text-foreground mb-4">Key Financial Ratios</h3>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="p-4 rounded-lg bg-muted/50">
              <div className="text-sm text-muted-foreground mb-1">Cash to Burn Ratio</div>
            ...
            <div className="p-4 rounded-lg bg-muted/50">
              <div className="text-sm text-muted-foreground mb-1">Revenue to Expense</div>
              <div className="text-2xl font-bold text-foreground">
                {Number(metrics.totalExpenses) > 0
                  ? ((Number(metrics.totalRevenue) / Number(metrics.totalExpenses)) * 100).toFixed(0)
                  : 0}%
              </div>
            ...
            <div className="p-4 rounded-lg bg-muted/50">
              ...
              <div className={`text-2xl font-bold ${Number(metrics.netCashflow) >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
            ...
            <div className="p-4 rounded-lg bg-muted/50">
              ...
              <div className={`text-2xl font-bold ${Number(metrics.growthRate) >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
