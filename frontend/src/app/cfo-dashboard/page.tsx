          ...
          <div className="w-full h-[calc(100vh-10rem)] flex items-center justify-center">
            <SpotlightCard className="max-w-md p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-6">
                Link your bank account via Plaid to unlock your CFO dashboard
                and gain real-time financial insights.
              </p>
              <PlaidLink
                onSuccess={handlePlaidSuccess}
                onError={(e: any) => toast.error(e.display_message)}
              />
            </SpotlightCard>
          </div>
        </MainLayout>
    ...
    },
    {
      ...
      content: (
        <div className="p-6">
          <div className="flex items-center gap-3">
            <div
              className={`h-10 w-10 rounded-lg ${cashFlow.netCashFlow >= 0 ? "bg-emerald-500/10 text-emerald-500" : "bg-red-500/10 text-red-500"} flex items-center justify-center`}
            >
              {cashFlow.netCashFlow >= 0 ? (
      ...
      content: (
        <div className="p-6">
          ...
          <div className="mt-4 text-4xl font-bold text-foreground">
            {cashFlow.runway > 0 ? (
              <>
                <CountUp to={Math.round(cashFlow.runway)} duration={1.5} /> days
              </>
            ) : (
              "N/A"
            )}
    ...
    },
    {
      ...
      content: (
        <div className="p-6">
          ...
          <AnimatedList
            items={accounts.breakdown.map((account) => (
              <div
                key={account.id}
