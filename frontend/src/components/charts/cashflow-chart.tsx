...
export default function CashflowChart({ data }: CashflowChartProps) {
  ...
  const chartData = data.map((item) => ({
    month: new Date(item.date + "-01").toLocaleDateString("en-IN", {
      month: "short",
    }),
    income: item.income,
  ...
  return (
    <BentoCard gradient glow>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="flex items-center gap-3">
          <div
            className={`h-10 w-10 rounded-xl ${netCashflow >= 0 ? "bg-gradient-to-br from-blue-500/20 to-cyan-500/20" : "bg-gradient-to-br from-orange-500/20 to-red-500/20"} flex items-center justify-center`}
          >
            <TrendingUp
              className={`h-5 w-5 ${netCashflow >= 0 ? "text-blue-400" : "text-orange-400"}`}
            />
          </div>
          <div>
            <p className="text-xs text-gray-400">Net Cashflow</p>
            <p
              className={`text-lg font-bold ${netCashflow >= 0 ? "text-blue-400" : "text-orange-400"}`}
            >
