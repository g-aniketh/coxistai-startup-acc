...
export async function analyzeVoucherAnomalies(
  ...
  const largeVouchers = vouchers.filter((v) => v.totalAmount.toNumber() > threshold);
  if (largeVouchers.length > 0) {
    alerts.push({
      type: 'anomaly',
      severity: 'warning',
      title: 'Unusually Large Transactions Detected',
      message: `Found ${largeVouchers.length} voucher(s) with amounts significantly above average (${avgAmount.toFixed(2)}).`,
  ...
  const vouchersWithIssues = vouchers.filter((v) => {
    ...
    if (v.entries.some((e) => !e.ledgerName || e.ledgerName.trim() === '')) return true;
    return false;
  });
  ...
  if (openai && vouchers.length >= 10) {
    try {
      const voucherSummary = {
        ...
        totalAmount: vouchers.reduce((sum, v) => sum + v.totalAmount.toNumber(), 0),
  ...
  "recommendations": ["recommendation 1", "recommendation 2"]
      ...
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a financial analyst analyzing accounting voucher data. Always respond in valid JSON format.',
          },
...
export async function detectVoucherVariances(
  ...
  for (const periodData of periods) {
    ...
    for (const [category, categoryVouchers] of Object.entries(byCategory)) {
      const actual = categoryVouchers.reduce((sum, v) => sum + v.totalAmount.toNumber(), 0);
...
export async function generateVoucherInsights(
  ...
  const summary = {
    ...
    byCategory: vouchers.reduce((acc, v) => {
      const category = v.voucherType.category;
...
${Object.entries(summary.byType)
  .map(([type, data]) => `  ${type}: ${data.count} vouchers, ${data.total.toFixed(2)} total`)
  .join('\n')}
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert CFO analyzing accounting voucher data. Provide clear, actionable insights. Always respond in valid JSON format.',
        },
        { role: 'user', content: prompt },
      ],
      temperature: 0.7,
