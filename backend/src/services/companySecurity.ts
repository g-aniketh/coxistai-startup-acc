...
export const upsertCompanySecurity = async (
  ...
  if (!existing) {
    ...
    if (toCreate.tallyVaultEnabled && !input.newTallyVaultPassword) {
      throw new Error(
        "TallyVault password is required when enabling encryption"
      );
    }
  ...
  if (
    typeof input.tallyVaultEnabled === "boolean" &&
    input.tallyVaultEnabled !== existing.tallyVaultEnabled
  ) {
    if (input.tallyVaultEnabled) {
      if (!input.newTallyVaultPassword) {
        throw new Error(
          "New TallyVault password is required to enable encryption"
        );
      }
      tallyVaultPasswordHash = await bcrypt.hash(
        input.newTallyVaultPassword,
        SALT_ROUNDS
      );
      ...
      if (existing.tallyVaultEnabled) {
        if (!input.currentTallyVaultPassword) {
          throw new Error(
            "Current TallyVault password is required to disable encryption"
          );
        }
        const valid = await bcrypt.compare(
          input.currentTallyVaultPassword,
          existing.tallyVaultPasswordHash || ""
        );
        if (!valid) {
  ...
  if (input.newTallyVaultPassword && tallyVaultEnabled) {
    if (!input.currentTallyVaultPassword) {
      throw new Error(
        "Current TallyVault password is required to rotate encryption password"
      );
    }
    const valid = await bcrypt.compare(
      input.currentTallyVaultPassword,
