    ...
    endpoints: {
      ...
      cfo: {
        plaid: {
          createLinkToken: {
            ...
            description:
              "Create Plaid link token for frontend integration (requires authentication)",
          },
          exchangePublicToken: {
            method: "POST",
            path: "/cfo/plaid/exchange-public-token",
            description:
              "Exchange public token for access token and store Plaid item (requires authentication)",
            body: {
          ...
          getItems: {
            ...
            description:
              "Get all Plaid items for the tenant (requires authentication)",
          },
          syncTransactions: {
            method: "POST",
            path: "/cfo/plaid/sync-transactions/:plaidItemId",
            description:
              "Sync transactions for a specific Plaid item (requires authentication)",
            body: {
          ...
          deleteItem: {
            ...
            description:
              "Delete a Plaid item and revoke access (requires authentication)",
          },
          webhook: {
            method: "POST",
            path: "/cfo/plaid/webhook",
            description:
              "Plaid webhook endpoint for transaction updates (no authentication required)",
          },
        },
        accounts: {
          method: "GET",
          path: "/cfo/accounts",
          description:
            "Get all financial accounts for the tenant (requires authentication)",
        },
        transactions: {
          method: "GET",
          path: "/cfo/transactions",
          description:
            "Get paginated transactions with filters (requires authentication)",
          query: {
        ...
        dashboard: {
          ...
          description:
            "Get dashboard summary with key financial metrics (requires authentication)",
          query: {
        ...
        categories: {
          ...
          path: "/cfo/categories",
          description:
            "Get all transaction categories (requires authentication)",
        },
        healthScore: {
          method: "GET",
          path: "/cfo/health-score",
          description:
            "Get financial health score and recommendations (requires authentication)",
          query: {
            period: "number (optional, days, default: 30)",
          },
        },
      ...
      tenants: {
        list: {
          ...
          path: "/tenants",
          description:
            "Get all tenants with their associated users (requires authentication)",
        },
        create: {
          method: "POST",
          path: "/tenants",
      ...
      users: {
        list: {
          ...
          path: "/users",
          description:
            "Get all users with their tenant information (requires authentication)",
        },
      },
    },
