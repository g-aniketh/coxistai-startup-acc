  ...
  async (req: AuthenticatedRequest, res: Response<ApiResponse>) => {
    try {
      ...
      const consistencyScore = Math.min(
        25,
        (transactionCount / expectedTransactions) * 25
      );
...
function generateRecommendations(
  ...
  if (totalBalance < 1000) {
    recommendations.push(
      "Build up emergency fund - aim for 3-6 months of expenses"
    );
  }
  ...
  if (
    factors.find((f) => f.name === "Transaction Consistency" && f.score < 15)
  ) {
    recommendations.push(
      "Ensure regular transaction monitoring and categorization"
    );
  }

  if (recommendations.length === 0) {
    recommendations.push(
      "Financial health looks good - continue current practices"
    );
  }
