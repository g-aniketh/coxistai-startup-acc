...
router.get('/registrations', async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      message: error instanceof Error ? error.message : 'Failed to fetch GST registrations',
    });
  }
});
...
router.post('/registrations', async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(400).json({
      success: false,
      message: error instanceof Error ? error.message : 'Failed to create GST registration',
    });
  }
});

router.put('/registrations/:registrationId', async (req: AuthRequest, res: Response) => {
  try {
    const startupId = req.user?.startupId;
    if (!startupId) {
      return res.status(400).json({ success: false, message: 'Startup context is required' });
    ...
    const registration = await updateGstRegistration(startupId, req.params.registrationId, req.body);
    return res.json({
      success: true,
      data: registration,
    ...
    return res.status(400).json({
      success: false,
      message: error instanceof Error ? error.message : 'Failed to update GST registration',
    });
  }
});

router.delete('/registrations/:registrationId', async (req: AuthRequest, res: Response) => {
  try {
    const startupId = req.user?.startupId;
    if (!startupId) {
      return res.status(400).json({ success: false, message: 'Startup context is required' });
    ...
    return res.status(400).json({
      success: false,
      message: error instanceof Error ? error.message : 'Failed to delete GST registration',
    });
  }
});
...
router.get('/tax-rates', async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      message: error instanceof Error ? error.message : 'Failed to fetch GST tax rates',
    });
  }
});
...
router.post('/tax-rates', async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(400).json({
      success: false,
      message: error instanceof Error ? error.message : 'Failed to create GST tax rate',
    });
  }
});
...
router.put('/tax-rates/:taxRateId', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const rate = await updateGstTaxRate(startupId, req.params.taxRateId, req.body);
    return res.json({
      success: true,
      data: rate,
    ...
    return res.status(400).json({
      success: false,
      message: error instanceof Error ? error.message : 'Failed to update GST tax rate',
    });
  }
});

router.delete('/tax-rates/:taxRateId', async (req: AuthRequest, res: Response) => {
  try {
    const startupId = req.user?.startupId;
    if (!startupId) {
      return res.status(400).json({ success: false, message: 'Startup context is required' });
    ...
    return res.status(400).json({
      success: false,
      message: error instanceof Error ? error.message : 'Failed to delete GST tax rate',
    });
  }
});
...
router.get('/ledger-mappings', async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      message: error instanceof Error ? error.message : 'Failed to fetch GST ledger mappings',
    });
  }
});
...
router.post('/ledger-mappings', async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(400).json({
      success: false,
      message: error instanceof Error ? error.message : 'Failed to create GST ledger mapping',
    });
  }
});

router.put('/ledger-mappings/:mappingId', async (req: AuthRequest, res: Response) => {
  try {
    const startupId = req.user?.startupId;
    if (!startupId) {
      return res.status(400).json({ success: false, message: 'Startup context is required' });
    ...
    const mapping = await updateGstLedgerMapping(startupId, req.params.mappingId, req.body);
    return res.json({
      success: true,
      data: mapping,
    ...
    return res.status(400).json({
      success: false,
      message: error instanceof Error ? error.message : 'Failed to update GST ledger mapping',
    });
  }
});

router.delete('/ledger-mappings/:mappingId', async (req: AuthRequest, res: Response) => {
  try {
    const startupId = req.user?.startupId;
    if (!startupId) {
      return res.status(400).json({ success: false, message: 'Startup context is required' });
    ...
    return res.status(400).json({
      success: false,
      message: error instanceof Error ? error.message : 'Failed to delete GST ledger mapping',
    });
  }
});
