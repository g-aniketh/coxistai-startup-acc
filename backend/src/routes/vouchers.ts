...
router.post("/types", async (req: Request, res: Response) => {
  try {
    ...
    if (!(category in VoucherCategory)) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid voucher category" });
    }
...
router.put("/types/:voucherTypeId", async (req: Request, res: Response) => {
  try {
    ...
    const voucherType = await updateVoucherType(
      startupId,
      voucherTypeId,
      payload
    );
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { voucherTypeId } = req.params;
      const series = await createVoucherNumberingSeries(
        startupId,
        voucherTypeId,
        req.body
      );
      ...
      return res.status(400).json({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to create numbering series",
      });
    }
  }
);
...
router.post(
  "/types/:voucherTypeId/next-number",
  async (req: Request, res: Response) => {
    try {
      const startupId = req.user?.startupId;
      if (!startupId) {
