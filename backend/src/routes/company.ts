...
import {
  getCompanyFiscal,
  upsertCompanyFiscal,
} from "../services/companyFiscal";
import {
  getCompanySecurity,
  upsertCompanySecurity,
} from "../services/companySecurity";
import {
  getCompanyCurrency,
  upsertCompanyCurrency,
} from "../services/companyCurrency";
import {
  getCompanyFeatureToggle,
  upsertCompanyFeatureToggle,
} from "../services/companyFeatureToggle";
    ...
    try {
      ...
      return res.status(500).json({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to fetch fiscal configuration",
      });
    }
  }
);
    ...
    try {
      ...
      return res.status(400).json({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to update fiscal configuration",
      });
    }
  }
);
    ...
    try {
      ...
      return res.status(500).json({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to fetch currency configuration",
      });
    }
  }
);
    ...
    try {
      ...
      return res.status(400).json({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to update currency configuration",
      });
    }
  }
);
    ...
    try {
      ...
      return res.status(500).json({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to fetch feature toggle configuration",
      });
    }
  }
);
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const featureToggle = await upsertCompanyFeatureToggle(
        startupId,
        req.body
      );
      ...
      return res.status(400).json({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to update feature toggle configuration",
      });
    }
  }
);
